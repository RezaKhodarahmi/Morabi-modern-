import { Badge, Button, Menu, Modal, Input } from 'antd';
...
class ImageMapEditor extends Component {
	...
	componentDidMount() {
		...
		if (this.props.editorId) {
			...
			fetch(routes.API.CANVAS.editor.replace(':id', this.props.editorId), {
				headers: {
				...
				response.json().then(data => {
					this.handlers.onImportFromJson(JSON.parse(data.object));
					this.setState({ editorName: data.name });
				});
	...
	canvasHandlers = {
		...
		onContext: (ref, event, target) => {
			...
			if (
				target.type === 'line' ||
				target.type === 'pathArrow' ||
				target.type === 'sineArrow' ||
				target.type === 'polygon'
			) {
				return (
					<Menu>
						<Menu.Item
							onClick={() => {
								this.canvasRef.handler.dashStroke();
							}}
						>
							{i18n.t('action.dash-line')}
						</Menu.Item>
						<Menu.Item
							onClick={() => {
								this.canvasRef.handler.remove();
	...
	handlers = {
		...
		getExportJson: () => {
			...
			return `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify(exportDatas, null, '\t'))}`;
		},
		onDownload: () => {
			const anchorEl = document.createElement('a');
			anchorEl.href = this.handlers.getExportJson();
		...
		onImportFromJson: json => {
			const { animations, styles, dataSources, objects } = json;
			this.setState({
				animations,
				styles,
				dataSources,
		...
		onFileSave: () => {
			...
			if (!this.state.editorName) return;
			body.append('file', this.canvasRef.handler.getCanvasImageBlob());
			body.append('json', this.handlers.getExportJson());
			body.append('name', this.state.editorName);
			if (this.props.editorId) body.append('editorId', this.props.editorId);
			fetch(routes.API.CANVAS.upload, {
				headers: {
					authorization: `Bearer ${localStorage.getItem('token')}`,
				},
				method: 'POST',
	...
	render() {
		...
		const action = (
			<React.Fragment>
				...
				<CommonButton
					...
					tooltipPlacement="bottomRight"
				/>
				<CommonButton onClick={() => this.setState({ visible: true })} tooltipPlacement="bottomRight">
					{i18n.t('action.file-save')}
				</CommonButton>
			</React.Fragment>
		);
		const titleContent = (
		...
		const title = <ImageMapTitle action={action} />;
		const content = (
				...
				>
					<div>
						<Input
							...
							placeholder={i18n.t('action.editor-name')}
						></Input>
						<Button onClick={onFileSave} block>
							{i18n.t('action.save')}
						</Button>
					</div>
				</Modal>
